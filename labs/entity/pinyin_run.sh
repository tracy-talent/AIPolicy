...
default_dropout=0.1 # 调整lexicon_window_size时使用
lexicon_window_sizes=(4 5 6)
...
python train_bert_bmes_lexicon_pinyin_freqasweight_mtl_span_attr_boundary.py \
    ...
    --word2vec_file ~/NLP/corpus/embedding/chinese/lexicon/ctbword_gigachar_mix.710k.50d.bin \
    --pinyin2vec_file ~/NLP/corpus/pinyin/word2vec/word2vec_num5.1409.200d.vec \
    --word2pinyin_file ~/NLP/corpus/pinyin/word2pinyin_num5.txt \
...
elif [ "$2" == "-grid" ]; then # 网格搜索dpr和wz
for _dpr in ${dropout_rates[*]}
do
  for _wz in ${lexicon_window_sizes[*]}
  do
    echo "Run dataset{$1}: dpr=$_dpr,wz=$_wz"
    PYTHONIOENCODING=utf8 \
    CUDA_VISIBLE_DEVICES=${GPU} \
    ${python_command} \
    --dropout_rate ${_dpr} \
    --lexicon_window_size ${_wz}
  done
done
...
#     --word2vec_file ~/NLP/corpus/embedding/chinese/lexicon/sgns_merge_word.1293k.300d.bin \