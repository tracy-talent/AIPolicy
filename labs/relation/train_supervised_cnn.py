...
logger = get_logger(sys.argv, os.path.join(config['path']['re_log'], f'{args.dataset}_cnn.log'))
...
tb_logdir = os.path.join(config['path']['re_tb'], f'{args.dataset}_cnn/{make_hparam_string(args.optimizer, args.lr, args.batch_size, args.weight_decay, args.max_length)}')
...
sentence_encoder = pasare.encoder.CNNEncoder(
    token2id=word2id,
