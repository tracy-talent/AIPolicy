...
class SentenceREDataset(data.Dataset):
    ...
    def collate_fn(cls, data):
        ...
        batch_labels = torch.tensor(labels).long()[sorted_length_indices] # (B)
        
        return [batch_labels] + seqs
...
class BagREDataset(data.Dataset):
    ...
    def collate_fn(cls, data):
        ...
        seqs_len = torch.cat(seqs[-1], dim=0).sum(dim=-1) # B
        sorted_length_indices = seqs_len.argsort(descending=True)
        unsorted_length_indices = sorted_length_indices.argsort() # 用于恢复原顺序
        seqs_len = seqs_len[sorted_length_indices]
        for i in range(len(seqs)):
            seqs[i] = torch.cat(seqs[i], dim=0)
            if seqs[i].size(1) > 0:
                seqs[i] = compress_seq(seqs[i][sorted_length_indices], seqs_len)
                seqs[i] = seqs[i][unsorted_length_indices].unsqueeze(0)
            else:
        ...
        return [label, bag_name, scope] + seqs # 不包含长度
    ...
    def collate_bag_size_fn(cls, data):
        ...
        for i in range(len(seqs)):
            if (seqs[i].size(1) > 0):
                seqs[i] = torch.cat(seqs[i], dim=0)
                seqs[i] = compress_seq(seqs[i][sorted_length_indices], seqs_len)
                seqs[i] = seqs[i][unsorted_length_indices].contiguous().view(len(label), -1, seqs[i].size(-1)) # (batch, bag, L)
            else:
        ...
        for c in count:
            ...
            start += c
        label = torch.tensor(label).long() # (B)
        return [label, bag_name, scope] + seqs # 不包含序列长度
