...
from .bert_encoder import BERTEntityEncoder, BERTEntityWithContextEncoder, BERTEntityWithDSPEncoder
...
class BERTEntPoolDistWithDSPEncoder(BERTEntityWithDSPEncoder):
    def __init__(self, pretrain_path, bert_name='bert', max_length=256, max_dsp_path_length=15, position_size=5, dsp_tool='ddp', 
                tag2id=None, use_attention4dsp=True, blank_padding=True, mask_entity=False, compress_seq=False, language='en'):
        super(BERTEntPoolDistWithDSPEncoder, self).__init__(pretrain_path=pretrain_path, 
                                                    bert_name=bert_name, 
                                                    max_length=max_length, 
                                                    max_dsp_path_length=max_dsp_path_length, 
                                                    dsp_tool=dsp_tool, 
    ...
    def forward(self, seqs, pos1, pos2, ent1_span, ent2_span, att_mask, ent_h_path, ent_t_path, ent_h_length, ent_t_length):
        """
        Args:
            seqs: (B, L), index of tokens
            pos1: (B, 1), position of the head entity starter
        ...
        # gather all features
        rep_out = torch.cat([ent1_avg_hidden, ent2_avg_hidden, dsp_hidden], dim=-1)  # (B, 4H)
        # rep_out = torch.tanh(self.linear(rep_out))
        rep_out = self.linear(rep_out)
    ...
    def tokenize(self, item):
        ret_items = super(BERTEntPoolDistWithDSPEncoder, self).tokenize(item)
        ent1_span = torch.zeros(ret_items[0].size())
        ent1_span[0, ret_items[1]:ret_items[3]] = 1
        ent2_span = torch.zeros(ret_items[0].size())
        ent2_span[0, ret_items[2]:ret_items[4]] = 1
        return ret_items[:5] + (ent1_span, ent2_span) + ret_items[5:]
...
class BERTEntityDistWithContextEncoder(BERTEntityWithContextEncoder):
    def __init__(self, pretrain_path, bert_name='bert', max_length=256, position_size=5, tag2id=None, 
                use_attention4context=True, blank_padding=True, mask_entity=False, language='en'):
        super(BERTEntityDistWithContextEncoder, self).__init__(pretrain_path=pretrain_path, 
                                                    bert_name=bert_name,
                                                    max_length=max_length, 
                                                    tag2id=tag2id, 
        ...
        self.pos1_embedding = nn.Embedding(2 * max_length, self.position_size, padding_idx=0)
        self.pos2_embedding = nn.Embedding(2 * max_length, self.position_size, padding_idx=0)                       
        emb_size = self.bert.config.hidden_size + self.position_size * 2
        self.hidden_size = emb_size * 3
...
class BERTEntityDistWithContextDSPEncoder(BERTEntityDistWithDSPEncoder):
    def __init__(self, pretrain_path, bert_name='bert', max_length=256, max_dsp_path_length=15, position_size=5, dsp_tool='ddp', tag2id=None, 
                use_attention4dsp=True, use_attention4context=True, blank_padding=True, mask_entity=False, compress_seq=False, language='en'):
        super(BERTEntityDistWithContextDSPEncoder, self).__init__(pretrain_path=pretrain_path, 
                                                    bert_name=bert_name, 
                                                    max_length=max_length, 
                                                    max_dsp_path_length=max_dsp_path_length, 
                                                    position_size=position_size, 
