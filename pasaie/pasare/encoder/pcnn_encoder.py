...
class PCNNEncoder(BaseEncoder):
    ...
    def __init__(self, 
        ...
        self.pool = PieceMaxPool(kernel_size=self.max_length, piece_num=3)
    ...
    def tokenize(self, item):
        ...
        pos1 = []
        pos2 = []
        pos1_in_index = min(pos_head[0], self.max_length - 1)
        pos2_in_index = min(pos_tail[0], self.max_length - 1)
        ...
        # Mask
        mask = []
        pos1_in_rindex = min(pos_head[1], self.max_length)
        pos2_in_rindex = min(pos_tail[1], self.max_length)
        pos_min = min(pos1_in_rindex, pos2_in_rindex)
        pos_max = max(pos1_in_rindex, pos2_in_rindex)
        for i in range(len(tokens)):
            if i < pos_min:
                mask.append(1)
            elif i < pos_max:
