...
class AutomaticWeightedLoss(nn.Module):
    ...
    def forward(self, *x):
        ...
        for i, loss in enumerate(x):
            if self.mode == 'cls':
                loss_sum += 2.0 / (self.params[i] ** 2) * loss + torch.log(self.params[i] ** 2 + 1) # +1 to avoid negtive
                #loss_sum += 2 * torch.exp(-self.params[i]) * loss + self.params[i]
            else:
                loss_sum += 1.0 / (self.params[i] ** 2) * loss + torch.log(self.params[i] ** 2 + 1) # +1 to avoid negtive
