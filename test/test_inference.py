...
class TestInference(unittest.TestCase):
    ...
    def test_policy_bmoes_bert_crf(sef):
        ...
        relation_model = pasaie.pasare.get_model('policy_bert_entity')
        while True:
            text = input()
            ...
            for i in range(len(entities)):
                for j in range(i + 1, len(entities)):
                    item = {'token':tokens, 'h': {'pos':[entities[i][0], entities[i][0] + len(entities[i][2])]}, 't': {'pos':[entities[j][0], entities[j][0] + len(entities[j][2])]}}
